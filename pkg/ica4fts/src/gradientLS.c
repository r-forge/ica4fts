...
/**Returns W[i,j]*/
double extractElement2(SEXP W,int *i,int *j){	
	int *Wdims;
	double *Wptr;
...
/**Uses logcosh and dlogcosh functions*/
SEXP gradientLS(SEXP Ag,SEXP E,SEXP S,SEXP H,SEXP N,SEXP PHI){
	...
	//Apply logCosh
	SEXP Hub; PROTECT(Hub = logCosh(S,&h)); //PROTECT 4
	double *hubptr; hubptr = REAL(Hub);	

	//Apply dlogCosh
	SEXP CP; PROTECT(CP = dlogCosh(S2,&h)); //PROTECT 5
	...
	int ell=0; //updated according to k
	...
	for(int j=1;j<=d;j++){	
			for(int i=1;i<=d;i++){	
				...
				if(j==J){													
					...
					dfdT = dfdT + dfdW*extractElement2(dWdT,&j,&i);	
			}//end for i
		}//end for j 
